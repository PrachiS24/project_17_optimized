...
int SpongeAbsorb(SpongeInstance *instance, const unsigned char data[200], size_t dataByteLen)
{
    size_t i, j;
    int loop;
    unsigned int partialBlock;
...
//                    SnP_AddBytes(instance->state, curData, 0, rateInBytes);
                    KeccakWidth1600_SpongeAbsorb_label10:for(loop=0;loop<rateInBytes;loop++)
                        SnP_AddByte(instance->state, data[loop], loop);
                    	//(instance->state) ## b
            ...
            for(loop=0;loop<partialBlock;loop++)
                SnP_AddByte(instance->state, data[loop], (instance->byteIOIndex)+loop);

//            SnP_AddBytes(instance->state, data, instance->byteIOIndex, partialBlock);
            data += partialBlock;
